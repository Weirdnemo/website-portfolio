...
export const BlogsResearchSection: React.FC = () => {
  return (
    <div id="blogs-research" className="py-10">
      <h2 className="font-medium text-primary/90 text-base">blogs &amp; research.</h2>

      {/* Blogs */}
      <h3 className="mt-6 text-muted-foreground text-xs uppercase tracking-wide">blogs</h3>
          ...
          <li key={post.title} className="cursor-target">
            <div className="pl-4 border-muted-foreground/40 hover:border-primary border-l size-full transition-all duration-300">
              <div className="flex sm:flex-row flex-col justify-between items-start">
                ...
                <span className="inline-block bg-secondary text-secondary-foreground max-sm:mb-2 ml-2 px-2 py-0.5 rounded text-xs whitespace-nowrap">
                  {post.date}
                </span>
              </div>
              ...
              <ul className="flex flex-wrap items-center gap-2 mt-2 pl-3">
                {post.tags.map((tag) => (
                  <li key={tag} className="bg-muted text-muted-foreground px-2 py-1 rounded text-xs">
                    {tag}
                  </li>
                ))}
              </ul>
      ...
      {/* Research */}
      <h3 className="mt-10 text-muted-foreground text-xs uppercase tracking-wide">research</h3>
      <ul className="flex flex-col gap-12 mt-4 font-normal text-primary/90 text-base">
        {portfolioData.research.map((paper) => (
          <li key={paper.title} className="cursor-target">
            <div className="pl-4 border-muted-foreground/40 hover:border-primary border-l size-full transition-all duration-300">
              ...
              <p className="text-muted-foreground text-sm mt-1">{paper.description}</p>
              ...
              {paper.url && (
                <div className="mt-2 pl-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 text-sm hover:underline underline-offset-2 w-fit"
                  >
                    View Paper
                    <ArrowUpRight className="w-4 h-4 ml-0.5" />
                  </a>
